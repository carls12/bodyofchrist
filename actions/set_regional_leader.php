...
$stmt = db()->prepare("SELECT is_regional_leader FROM users WHERE id=?");
$stmt->execute([$userId]);
$row = $stmt->fetch();
if (!$row) { redirect(base_url('admin/users')); }

if ($action === 'remove') {
  db()->prepare("UPDATE users SET is_regional_leader=0 WHERE id=?")->execute([$userId]);
} else {
  db()->prepare("UPDATE users SET is_regional_leader=1, region=? WHERE id=?")
    ->execute([$region ?: null, $userId]);
}
